...
### Server-Side Exploit Selection -- NoVoice (2026)

[NoVoice](../malware/families/novoice.md) introduced a server-side exploit delivery model for commodity malware. Rather than bundling exploits in the APK, the malware [sends the device's chipset, kernel version, security patch date, and hardware identifiers to the C2](https://www.mcafee.com/blogs/other-blogs/mcafee-labs/new-research-operation-novoice-rootkit-malware-android/), which responds with exploit binaries matched to that specific device. McAfee recovered 22 exploits from the C2, targeting vulnerabilities patched between 2016 and 2021. One analyzed chain used an IPv6 use-after-free for kernel read, a Mali GPU driver vulnerability for kernel read/write, then credential patching and SELinux disablement. This approach previously required commercial spyware-level infrastructure but was deployed at scale through 50+ Play Store apps with 2.3M+ downloads, targeting budget devices in regions where security patches lag.
