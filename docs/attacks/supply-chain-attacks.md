...
## Repackaging Toolchains

A different supply-chain shape: rather than poisoning a developer's build, the attacker takes a finished APK off the Play Store, modifies it on-device, and redistributes through warez stores and Telegram channels. The result is a legitimate-looking app whose signing key has been replaced and whose Java/native code has been silently extended with adware, credential capture, or full RAT functionality. The dominant tooling sits on Android itself, in Chinese modding communities, and produces a recognizable fingerprint set that telemetry can pivot on.

| Tool | Role | Recurring Fingerprints |
|------|------|------------------------|
| [MT Manager](https://mt2.cn/) (`bin.mt.plus`) | On-device APK/Dex/ARSC/XML editor; the dominant Chinese modding workbench | `bin.mt.signature` injected helper class, `smalihelper` smali patterns, MT's own signer key on resigned APKs |
